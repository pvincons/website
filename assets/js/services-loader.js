document.addEventListener('DOMContentLoaded', () => {
    fetch('content/posts-services.json')
        ...
        .then(services => {
            // Xóa HTML cũ
            document.getElementById('pricing-list-container').innerHTML = '';
...
function renderServices(services) {
    services.forEach(item => {
        // Tìm container dựa trên category
        const container = document.getElementById(`${item.category}-list-container`);
                ...
                serviceHTML = `
                    <div class="p-5 bg-white rounded-xl border border-slate-200 flex justify-between items-stretch shadow-sm hover:shadow-md hover:border-brand-blue/40 transition-all gap-3">
                        <div class="min-w-0 flex-1 flex flex-col justify-center">
                            ...
                            <p class="text-xs text-slate-500 mt-1"><i class="fa-solid ${item.icon} mr-1 text-brand-orange"></i>${item.summary}</p>
                        </div>
                        <div class="flex flex-col justify-between gap-2 shrink-0 w-32 sm:w-36">
                            <button onclick="openRequestModal('${item.title}', 'Tải báo giá')" class="w-full text-brand-blue hover:text-brand-orange text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 transition-colors py-2 px-3 rounded-lg bg-slate-50 border border-slate-200 hover:border-brand-orange whitespace-nowrap cursor-pointer"><i class="fa-solid fa-download"></i> Tải báo giá</button>
                            <a href="${item.link}" class="w-full text-brand-blue hover:text-brand-orange text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 transition-colors py-2 px-3 rounded-lg bg-slate-50 border border-slate-200 hover:border-brand-orange whitespace-nowrap"><i class="fa-solid fa-eye"></i> Xem quy trình</a>
                        </div>
                    </div>
                `;
